...
function ToDos({ todoList }) {
  ...
  return (
    <div data-test="component-toDos">
      ...
      <div data-test="toDos">
        {todoList &&
          todoList.map(({ todo, completed }, index) => {
            return (
              //id and className has been added for future reference. not needed for now.
              <div key={index} id={index} className="todo" data-test="todo">
                <span
                  data-test="title"
                  className={!completed ? "incomplete" : "complete"}
                >
                  {todo}
                </span>
                <span
                  data-test="individualToDoCompleteStatus"
                  id={`todo-${index}`}
                  onClick={() => {
